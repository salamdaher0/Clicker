...
<?php 
if(!isset($_GET['id'])){
	//Redirect user to index.php shell if this page is accessed directly
	$request_uri = $_SERVER['REQUEST_URI'];					//Get the request (the part after www.vcom3d.com)
	$request_array = explode('/', $request_uri);			//Split string by directories
	$filename = $request_array[count($request_array) - 1];	//Isolate the last part (file name)
	$filename_array = explode('.', $filename);				//Separate filename from extension
	header("Location: index.php?id=" . $filename_array[0]);	//Redirect user to index.php using filename without extension
}
...
<div id="container">	
	...
	<p> Please use this form to send an email to Salam Daher. Thank you <p>	
...
<div class="left">
   ...
   <form id="contactUs" action="sendmail.php" method="post" onsubmit= "return container.validateAll();">
